...
const fragmentShader = `
  ...
  void main() {
    // Simple liquid metal iridescence effect
    vec3 viewDirection = normalize(cameraPosition - vNormal);
    float fresnel = dot(viewDirection, vNormal);
    fresnel = clamp(1.0 - fresnel, 0.0, 1.0);
    fresnel = pow(fresnel, 3.0);

    vec3 color1 = vec3(0.1, 0.1, 0.12);
    vec3 color2 = vec3(0.8, 0.8, 0.9);

    vec3 finalColor = mix(color1, color2, fresnel);

    // Add some subtle time-based color shifting
    finalColor.r += sin(uTime * 0.5 + vUv.x * 10.0) * 0.1;
    finalColor.b += cos(uTime * 0.3 + vUv.y * 10.0) * 0.1;

    gl_FragColor = vec4(finalColor, 1.0);
  }
`;

function LiquidMetalIcosahedron() {
